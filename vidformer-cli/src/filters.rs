...
use std::ptr;
use std::sync::Arc;
use vidformer::filter::*;
use vidformer::AVFrame;
use vidformer::Error;

fn avfilter_backed_uniframe(frame: &Arc<AVFrame>, filter: &str) -> Result<Frame, vidformer::Error> {
...
fn opencv_backed_uniframe<F>(input_frame: &Frame, filter: F) -> Result<Frame, vidformer::Error>
where
    F: FnOnce(&mut Mat),
{
...
impl Filter for Pad {
    ...
    fn filter_type(
        ...
        args: &[ValType],
        kwargs: &BTreeMap<String, ValType>,
    ) -> Result<FrameType, vidformer::Error> {
...
macro_rules! create_filter {
    ($struct_name:ident, $func:ident, $tyfunc:ident, { $($arg_name:ident: $arg_type:ident $req:ident),* }, { $($kwarg_name:ident: $kwarg_type:ident $kwreq:ident),* }) => {
        impl Filter for $struct_name {
            fn filter(&self, args: &[Val], kwargs: &BTreeMap<String, Val>) -> Result<Frame, vidformer::Error> {
